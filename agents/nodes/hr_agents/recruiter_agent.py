...
class RecruiterAgent(Runnable):
    def __init__(self, llm=None, seuil_min_effectif: int = 5):
        self.llm = llm or LLMManager().get_llm()
        self.logger = logging.getLogger(__name__)
        self.seuil_min_effectif = seuil_min_effectif
        self.distance_max_km = 50
    ...
    def invoke(self, input: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        try:
            ...
            for c in capacites_disponibles:
                ...
                if effectif >= self.seuil_min_effectif:
            ...
            if not competences_recherchees:
                self.logger.warning("Aucune compétence extraite via la requête. On récupère depuis data_analytics.")
            ...
            self.logger.error(f"[RecruiterAgent] Erreur : {str(e)}", exc_info=True)
            return {
                "status": "error",
                "profils": [],
                "error": str(e)[:200]
